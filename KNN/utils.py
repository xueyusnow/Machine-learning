...
class Distances:
    ...
    def minkowski_distance(point1, point2):
        ...
        """
        #raise NotImplementedError
        return np.power(np.sum(np.power(np.abs(np.array(point1)-np.array(point2)),3)),1.0/3)
...
class HyperparameterTuner:
    ...
    def tuning_without_scaling(self, distance_funcs, x_train, y_train, x_val, y_val):
        ...
        k=0
        #self.best_model = None
        #self.best_distance_function=""
        for curname,curfunc in distance_funcs.items():
            for i in range(1,min(len(x_train),31),2):
                curmodel=KNN(i,curfunc)
                curmodel.train(x_train,y_train)
                pred_val=curmodel.predict(x_val)
                curf1=f1_score(y_val,pred_val)
                if curf1>f1:
                    f1=curf1
                    k=i
                    name=curname
    ...
    def tuning_with_scaling(self, distance_funcs, scaling_classes, x_train, y_train, x_val, y_val):
        ...
        #self.best_distance_function=""
        #self.best_scaler=""
        for curscalername,curscaler in scaling_classes.items():           
            for curname,curfunc in distance_funcs.items():
                scaler=curscaler()
                trainscale=scaler(x_train)
                validscale=scaler(x_val) 
                for i in range(1,min(len(x_train),31),2):
                    curmodel=KNN(i,curfunc)
                    curmodel.train(trainscale,y_train)
                    pred_val=curmodel.predict(validscale)
                    curf1=f1_score(y_val,pred_val) 
                    if curf1>f1:
                        f1=curf1
                        k=i
                        name=curname
                        model=curmodel
                        scalern=curscalername
        
        self.best_k = k
        self.best_distance_function = name
        self.best_scaler = scalern
        self.best_model = model
        ...
        for point in v1:
            if np.linalg.norm(point)==0:
                ...
                for i in range(len(point)):
                    p.append(point[i]/np.linalg.norm(point))
                c.append(p)
        return c
    ...
    def __call__(self, features):
        ...
        for i in range(len(v1[0])):
            ...
            for j in range(len(v1)):
                mini=min(v1[j][i],mini)
                maxi=max(v1[j][i],maxi)
            minv.append(mini)
            maxv.append(maxi)
        c=[]
        for i in range(len(v1)):
            ...
            for j in range(len(v1[0])):
                if maxv[j]-minv[j]==0:
                    ...
                    row.append((v1[i][j]-minv[j])/(maxv[j]-minv[j]))
            c.append(row)
        return c