...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    if loss == "perceptron":
        ...
        for i in range(max_iterations):
            ywx = np.where(((np.dot(w,X.T) + b) * y) <= 0, 1, 0)
            gradientw=np.dot((ywx * y),X)/N
            gradientb=np.sum(ywx * y)/N
        ...
        ################################################
        for i in range(max_iterations):
            loss=sigmoid(np.dot(w,X.T)+b)-y
            gradientw=np.dot(loss,X)/N
            gradientb=np.sum(loss)/N
            w=w-step_size*gradientw
            b=b-step_size*gradientb
...
def multiclass_train(X, y, C,
    ...
    if gd_type == "sgd":
        ...
        for it in range(max_iterations):
            ...
            ####################################################
            #Numerator
            wkk = np.dot(w, X[n].T) + b.T
            wk = wkk - wkk.max()
            wn = np.exp(wk)
            p = wn/np.sum(wn)
            p[y[n]] = p[y[n]] - 1
            gradientw=np.dot(p.reshape(C, 1), X[n].reshape(1, D))
            gradientb=p
            w=w-step_size*gradientw
            b=b-step_size*gradientb
        ...
        for i in range(N):
            yy[i,y[i]]=1
        for it in range(max_iterations):
            wkk = np.dot(w, X.T) + b.reshape(C,1)
            wk = wkk - wkk.max()
            wn = np.exp(wk)
            p = wn/np.sum(wn,axis=0)-yy.T
            gradientw=np.dot(p, X)
            gradientb=np.sum(p,axis=1)
            w=w-step_size*gradientw/N
            b=b-step_size*gradientb/N
