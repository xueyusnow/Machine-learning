...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    for j in range(n_cluster-1):
        ...
        for i in range(n):
            if i not in centers:
                p[i]=min((np.linalg.norm(x[i]-x[c])**2) for c in centers)
        prob=p/np.sum(p)
        center = np.argmax(np.cumsum(prob) >= r)
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        oldcentroids=x[self.centers]
        diff1=pow(100,4)
        for t in range(self.max_iter):
            membership = np.argmin([np.linalg.norm(x-centroids[i], axis=1) ** 2 for i in range(self.n_cluster)], axis=0)
            diff2 = np.sum([np.linalg.norm(x[membership==i]-centroids[i]) ** 2 for i in range(self.n_cluster)], axis=0)/N
            if np.absolute(diff1-diff2)<=self.e:
                break
            diff1=diff2
            centroids = np.array([np.mean(x[membership==i], axis=0) for i in range(self.n_cluster)])
            np.where(centroids == None,oldcentroids,centroids)
            oldcentroids=centroids.copy()
        return centroids, membership, t+1
...
class KMeansClassifier():
    ...
    def fit(self, x, y, centroid_func=get_lloyd_k_means):
        ...
        centroids, membership, iters = kmeans.fit(x, centroid_func)
        centroid_labels = np.zeros(self.n_cluster)

        for i in range(self.n_cluster):
            nums = np.where(membership == i)
            unique, counts = np.unique(y[nums], return_counts=True)  
    ...
    def predict(self, x):
        ...
        labels = np.zeros(N,)
        for n in range(N):
            labels[n] = self.centroid_labels[np.argmin([(np.linalg.norm(x[n]-self.centroids[i])**2) for i in range(self.n_cluster)])]
        return labels
...
def transform_image(image, code_vectors):
    ...
    ##############################################################################
    X,Y,Z=image.shape
    nums = np.shape(code_vectors)[0]
    newc = np.argmin([np.linalg.norm(image-code_vectors[i], axis=2) for i in range(nums)], axis=0)
    newimage = code_vectors[newc].reshape(X,Y,Z)
